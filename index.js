...
async function run() {
  try {
    ...
    // app.get('/products', async (req, res) => {
    //   const { page = 1, limit = 9, search = '', brand = '', category = '', priceRange = '', sort = 'price', order = 'asc' } = req.query;
    ...
    //   }
    //   if (priceRange) {
    //     // Ensure priceRange is in the format "$min - $max"
    //     const [minPrice, maxPrice] = priceRange.replace('$', '').split(' - ').map(Number);
    ...
    //   // Apply sorting only if a valid sort field is provided
    //   if (sort) {
    //     const sortOrder = order === 'desc' ? -1 : 1;
    ...
    //     const products = await productsSort.find(filters)
